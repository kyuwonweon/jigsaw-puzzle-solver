"""Template matching for jigsaw puzzle pieces using SIFT and NCC methods."""
...
def try_match(sift, kp_bg, des_bg, img, mask, idx, label, ratio=0.75):
    """Match template keypoints to background using SIFT descriptors with Lowe's ratio test.

    Args:
        sift: SIFT detector instance.
        kp_bg: Background keypoints.
        des_bg: Background descriptors.
        img: Template image to match.
        mask: Alpha channel mask for the template.
        idx: Piece index for error reporting.
        label: Stage label for error reporting.
        ratio: Lowe's ratio threshold (default 0.75).

    Returns:
        Tuple of (H, n_inliers, error_msg) where H is the homography matrix,
        n_inliers is the count of inlier matches, or (None, None, error_msg) on failure.
    """
    kp_t, des_t = sift.detectAndCompute(img, mask)
    if des_t is None or len(kp_t) < 4:
        n = len(kp_t) if kp_t else 0
        return None, None, f'piece_{idx:02d} [{label}]: only {n} keypoints'
...
def project_bbox(H, mask, tmpl_wh, bg_shape, idx, label):
    """Project template mask through homography to get bounding box on background.

    Args:
        H: Homography matrix from template to background space.
        mask: Alpha channel mask for extracting contours.
        tmpl_wh: Template dimensions (width, height).
        bg_shape: Background image shape (height, width, channels).
        idx: Piece index for error reporting.
        label: Stage label for error reporting.

    Returns:
        Tuple of (bbox, error_msg) where bbox is (x0, y0, width, height) or None on failure.
    """
    tw, th = tmpl_wh
    bH, bW = bg_shape[:2]
    ...
    if pw <= 0 or ph <= 0:
        return None, f'piece_{idx:02d} [{label}]: degenerate bbox ({pw}x{ph})'
    ...
    if not (0.3 < (pw * ph) / (tw * th) < 2.0):
        return None, f'piece_{idx:02d} [{label}]: size {pw}x{ph} unreasonable vs {tw}x{th}'
...
for tf in sorted(templates_dir.glob('piece_*.png')):
    ...
    for sift, kp_bg, des_bg, ch_key, label, ratio in stages:
        H, n_in, err = try_match(
            sift, kp_bg, des_bg, channels[ch_key], mask, idx, label, ratio
        )
        if H is None:
            last_err = err
            continue
        ...
        if bbox is None:
            ...
            continue
        x0, y0, pw, ph = bbox
        print(f'piece_{idx:02d}: bbox ({x0:4d},{y0:4d}) {pw}x{ph}  inliers={n_in}  [{label}]')
    ...
    if not matched:
        ...
        if bbox is not None:
            x0, y0, pw, ph = bbox
            print(f'piece_{idx:02d}: bbox ({x0:4d},{y0:4d}) {pw}x{ph}  [ncc]')
            results.append((idx, x0, y0, pw, ph, 0))
        else:
            print(err)

# Post-process: normalize scale using high-confidence SIFT results
hi_scales = [
    ...
    for r in results:
        ...
        if idx in tmpl_size:
            ...
            if n_in == 0:
                ...
                tmpl = cv2.imread(
                    str(templates_dir / f'piece_{idx:02d}.png'), cv2.IMREAD_UNCHANGED
                )
                ...
                if bbox is not None:
                    ...
                    n_in = 0
                    print(
                        f'piece_{idx:02d}: scale {scale:.2f} > {ref_scale*1.3:.2f}'
                        f' — re-matched with NCC → ({x0},{y0}) {pw}x{ph}'
                    )
                ...
                pw, ph = new_w, new_h
                print(f'piece_{idx:02d}: scale {scale:.2f} — resized to {pw}x{ph}')
                r = (idx, x0, y0, pw, ph, n_in)
        refined.append(r)
    results = refined
